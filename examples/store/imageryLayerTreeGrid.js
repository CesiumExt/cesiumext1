...
Ext.application({
    ...
    launch: function() {
		...
		mapComponent.on('viewercreated', function(viewer) {
			...
			//create tree panel
			var treeGridPanel = createImageryLayerTreeGridPanel(imageryLayerTreeStore);
			//create tree panel
		...
		function createImageryLayerTreeGridPanel(treeStore) {
			...
			var treePanel = Ext.create('Ext.tree.Panel', {
				title: 'ImageryLayer Tree Grid Panel',
				columns: [
					{
						xtype: 'treecolumn',
						dataIndex: 'text',
			...
			treePanel.on('validateedit', function (editor, context) {
				var imageryLayer = context.record.getCesiumImageryLayer();
				if(Cesium.defined(context.newValues.alpha))
					imageryLayer.alpha = context.newValues.alpha;
				if(Cesium.defined(context.newValues.brightness))
					imageryLayer.brightness = context.newValues.brightness;
				if(Cesium.defined(context.newValues.contrast))
					imageryLayer.contrast = context.newValues.contrast;
				if(Cesium.defined(context.newValues.hue))
					imageryLayer.hue = context.newValues.hue;
				if(Cesium.defined(context.newValues.saturation))
					imageryLayer.saturation = context.newValues.saturation;
				if(Cesium.defined(context.newValues.gamma))
					imageryLayer.gamma = context.newValues.gamma;
				if(Cesium.defined(context.newValues.show))
			...
			tabPanel.add(treePanel).show();
			tabPanel.setActiveTab(treePanel);
